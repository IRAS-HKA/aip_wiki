...
5. **Option 2: Create new project package**
   ...
   - Load the IOs by using the integrate scan function
      ...
      <img src="../images/20240228_DeviceScan.png" width="500">
...
6. **Create a new safety project**
   ...
   - Create new safety project part
   ...
   - Create TwinSafeGroup
     ...
     - Parameterisation of the individual modules
        - Import the necessary alias devices from the current IO configuration
        ...
        - Create a SafeEstop building block by drag and drop from the toolbox
          ...
          - Map the variables to the corresponding channels of the modules or further variables used
            ...
            - **Output side:**
                ...
                Note: _To communicate with the KUKA KR10 robot, the process image must from Beckhoff PLC and KUKA KR C4 control must match. If you want to e.g. set status lamps, it is necessary to report to a variable im regular PLC part._
          ...
          - Insert a function block "safeDecouple" from the toolbox and configure it to have the E-Stop of the SmartPad available in the "normal" PLC. For this create an alias device and map it with the function block.

            <img src="../images/20240228_safeDecouple.png" width="400">
        ...
        - Load the configuration on the physical safety modules
          ...
          - Click _"TwinSAFE/Mehrere Safety Projekte herunterladen"_
            ...
            <img src="../images/20240228_ProjekteHerunterladen.png" width="400">
...
7. **Create new standard PLC project**
   ...
   - Create programmble object units (POUs)
     - The _main_ has to call the PRG_Safety POU in order to run it on the PLC  
        ...
        It generates a PLC task im main (see PLC tasks, PLC). Cyclus ticks of the PLC can be reviewed via System/Tasks/PLCTasks.

   - **PRG_Safety**
     - Read Inputs-/ Ouputs of the safety PLC

        Note: _Outputs of Safety PLC are inputs of the standard PLC._
   ...
   - Set status lamp for operating status display
      ...
      ```bash
        ...
        // Lamp YELLOW      
        ELSIF InputNotHaltCHA AND InputNotHaltCHB AND InputLidarCHA AND  InputLidarCHB AND NotHalt_SmartPad = TRUE
          THEN 
      ...
      <img src="../images/20240222_PLC_standard_status_lamps_code.png" width="900">
     ...
     - Connect them to the physical hardware in the IO topology (e.g. "_LampeNotHalt_OK_")  
      ...
      <img src="../images/20240222_PLC_standard_status_lamps_variables_connect_to_physical_hardware.png" width="400">
   ...
   - Connect the PLC variables with the physical inputs / outputs
...
8. **Adjust the process image of the KUKA KR C4 module in the IO structure**
   - Select the box of the KR C4 in the tab "_Slots_"
     - Choose the corresponding process image ("Safety Daten"/ "Reguläre Daten"/ "Kombiniert")
        ...
        <img src="../images/20240222_PLC_KRC4_adjust_process_image.png" width="400">
