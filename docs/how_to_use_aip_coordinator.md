...
Please note: The [Gripper.cpp](https://github.com/IRAS-HKA/aip_cell_description/blob/main/aip_bosch_gripper/src/gripper.cpp) is located in the [aip_bringup](https://github.com/IRAS-HKA/aip_bringup) repository, not in the AIP_Coordinator repository.

#### Mapping from cylinder id to the hardware pins

To enable an operation mode with different modes, the .cpp file contains a mapping from the cylinder_id to the corresponding hardware pins.
...
MoveGripper requests parameter from the method-head (variable name "request"), contains the necessary pins to perform the desired movement of the gripper.
